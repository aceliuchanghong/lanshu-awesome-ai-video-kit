---
name: model-selector
description: 根据用户的视频需求（场景、时长、音频、语言、平台限制、预算等），从 11 个主流 AI 视频模型中推荐最匹配的 1-3 个，并解释为什么。覆盖 Seedance、HappyHorse、Kling、Sora 2、Veo 3、Runway Gen-4/Aleph、Pika 2.5、Hailuo 02、Hunyuan Video、Wan 2.5、即梦 3.0。用于"用哪个 AI 视频模型好"、"Sora 还是 Kling"、"国内有什么模型"、"哪个免费"、"哪个支持中文"、"哪个能编辑已有视频"等触发场景。
---

# model-selector

11 个主流 AI 视频模型的"购物顾问"。问用户 3-4 个关键问题，给出推荐 + 理由。
...
>
> **备选**：**Wan 2.5**（如果对话很多且需要数字人感强）/ **即梦 3.0**（如果在剪映里直接做后期）
